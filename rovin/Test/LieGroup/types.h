...
inline SRreal DEG2RAD(int d)	 { return (d * SR_RADIAN); }
inline SRreal RAD2DEG(int r)	 { return (r * SR_DEGREE); }
inline SRreal DEG2RAD(float d) { return (d * SR_RADIAN); }
inline SRreal RAD2DEG(float r) { return (r * SR_DEGREE); }
inline SRreal DEG2RAD(double d){ return (d * SR_RADIAN); }
inline SRreal RAD2DEG(double r){ return (r * SR_DEGREE); }
...
#define XXSAFE_DELETE(p)		if(p) { delete (p); (p) = NULL; }
#define XXSAFE_DELETE_AR(p)		if(p) { delete [] p; (p) = NULL; }
