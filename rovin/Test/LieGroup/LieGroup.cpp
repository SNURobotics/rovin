...
{
	SRreal mass = (SRreal)8.0 * density * size[0] * size[1] * size[2];
...
{
	SRreal mass = density * SR_FOURTHIRD * SR_PI * rad * rad * rad;
	SRreal i = (SRreal)0.4 * mass * rad * rad;
...
{
	rad *= rad;
	SRreal mass = density * SR_PI * rad * height;
	SRreal ix = mass * height * height / (SRreal)12.0 + (SRreal)0.25 * mass * rad;
...
{
	SRreal cyl_mass = density * SR_PI * rad * rad * height;
	SRreal sph_mass	= density * SR_FOURTHIRD * SR_PI * rad * rad * rad;

	SRreal ix = cyl_mass * height * height / (SRreal)12.0 + (SRreal)0.25 * cyl_mass * rad			// cylinder
				+ (SRreal)0.4 * sph_mass * rad * rad + (SRreal)0.25 * sph_mass * height * height;	// sphere

	return Inertia(
		cyl_mass + sph_mass,													// mass
		ix,																		// ix
		ix,																		// iy
		0.5 * cyl_mass * rad * rad + (SRreal)0.4 * sph_mass * rad * rad	// iz
		);
}
...
Quaternion::Quaternion(const Vec3& axis, SRreal angle)
{
	SRreal d, s, theta;
...
Quaternion::Quaternion(const SE3& m)
{
	...
	if (tr > (SRreal)0.0)
	{
		...
		j = nxt[i];
		k = nxt[j];
		s = sqrt((m[i*4] - (m[j*4] + m[k*4])) + (SRreal)1.0);
		c[i] = s * (SRreal)0.5;
...
Quaternion operator * (SRreal k, const Quaternion& q)
{
...
SRreal dot(const Quaternion& p, const Quaternion& q)
{
...
Quaternion slerp(const Quaternion& p, const Quaternion& q, SRreal t)
{
	SRreal theta, sintheta, costheta, pscl, qscl = 1.0f;
