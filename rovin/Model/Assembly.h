		...
		public:
			...
			*/
			...
			public:
				...
				std::shared_ptr<Joint> _joint_pointer;
				std::shared_ptr<Link> _mountLink_pointer;
				std::shared_ptr<Link> _actionLink_pointer;
				Math::SE3 _mountLink_T;
				Math::SE3 _actionLink_T;
			};
			...
			void addJoint(const std::shared_ptr<Joint>& joint_pointer, ///< 추가하고 싶은 조인트의 포인터
				...
				const Math::SE3& actionLink_T = (Math::SE3()) ///< Action 링크 프레임에서 바라본 joint 프레임
				)
			{
			...
			void addJoint(const std::shared_ptr<Joint>& joint_pointer, ///< 추가하고 싶은 조인트의 포인터
				...
				const Math::SE3& mountLink_T = (Math::SE3()), ///< Mount 링크 프레임에서 바라본 joint 프레임
				const Math::SE3& actionLM_T = (Math::SE3()) ///< Action 링크 또는 마커 프레임에서 바라본 joint 프레임
				)
			{
